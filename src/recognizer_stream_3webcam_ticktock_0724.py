...
#detector = MtcnnDetector(model_folder="/home/kucg/Recognizer_face_and_hand_gesture/insightface/deploy/mtcnn-model")
...
while True:
    ...
    if frames % 5 == 0:
        ...
        if not ret :
            print("cam 0 disconnected, connecting again : %d"%ret)
            cap = cv2.VideoCapture(0)
            continue
        if not retIn : 
            print("cam 1 disconnected, connecting again : %d"%retIn)
            capIn = cv2.VideoCapture(1)
            continue
        if not retIn2 :
            print("cam 2 disconnected, connecting again : %d"%retIn2)
            capIn2 = cv2.VideoCapture(2) 
