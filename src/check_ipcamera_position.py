...
while True:
    ...
    if frames % 5 == 0:
        ...
        cnt = []
        max_bbox = None
        facedetect_tick = time.time()
        bboxes_Out = detector.detect_faces(frame)
        facedetect_tock = time.time()
        handdetect_tick = time.time()
        draw_points, points, hand_box, hand_area = hand_detector(rgb)
        handdetect_tock = time.time()
        #trackers = []
        ...
        print("opencv detection time: {}s".format(opencv_tock-opencv_tick))
        print("Faces recognition time: {}s".format(facedetect_tock-facedetect_tick))
        print("hand recognition time: {}s".format(handdetect_tock-handdetect_tick))
