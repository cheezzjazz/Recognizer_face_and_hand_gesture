...
rtsp1 = 'rtsp://admin:**graphics@192.168.0.12:554/cam/realmonitor?channel1&subtype=1'
rtsp2 = 'rtsp://admin:**graphics@192.168.0.16:554/cam/realmonitor?channel1&subtype=1'
rtsp3 = 'rtsp://admin:**graphics@192.168.0.18:554/cam/realmonitor?channel1&subtype=1'
cap1Time = time.time()
cap = cv2.VideoCapture(rtsp1) #output camera
cap1Time2 = time.time()

cap2Time = time.time()
capIn = cv2.VideoCapture(rtsp2) #input camera1
cap2Time2 = time.time()
...
cap3Time = time.time()
capIn2 = cv2.VideoCapture(rtsp3) #input camera2
cap3Time2 = time.time()
...
for i in range(40):
    ...
    ret, frame = cap.retrieve()
    ret = capIn.grab()
    ret, frameIn = capIn.retrieve()
"""
cap = cv2.VideoCapture(args.video_in)
capIn = cv2.VideoCapture(args.video_in)
capIn2 = cv2.VideoCapture(args.video_in)

cap.set(cv2.CAP_PROP_BUFFERSIZE, 10)
capIn.set(cv2.CAP_PROP_BUFFERSIZE, 10)
capIn2.set(cv2.CAP_PROP_BUFFERSIZE, 10)

#cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter.fourcc('Y','1','6',' '))
#cap.set(cv2.CAP_PROP_CONVERT_RGB, False)

#capIn.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter.fourcc('Y','1','6',' '))
#capIn.set(cv2.CAP_PROP_CONVERT_RGB, False)

#capIn2.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter.fourcc('Y','1','6',' '))
#capIn2.set(cv2.CAP_PROP_CONVERT_RGB, False)
...
"""
CamTime2 = time.time()
print("Cam1 Load time: {}s".format(cap1Time2 - cap1Time))
print("Cam2 Load time: {}s".format(cap2Time2 - cap2Time))
print("Cam3 Load time: {}s".format(cap3Time2 - cap3Time))
print("Cam Start time: {}s".format(CamTime2 - CamTime1))
"""
while True:
    # ret is fading
    ret = cap.grab()        #output camera
    retIn = capIn.grab()     #input camera1
    retIn2 = capIn2.grab()   #input camera2

    ret, frame = cap.retrieve()
    retIn, frame_In = capIn.retrieve()
    retIn2, frame_In2 = capIn2.retrieve()
    ...
    if True:
        ...
        if not retIn : 
            print("capIn : %d"%retIn)
            capIn = cv2.VideoCapture(rtsp2)
            #capIn.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter.fourcc('Y','1','6',' '))
            #capIn.set(cv2.CAP_PROP_CONVERT_RGB, False)
            continue
        if not retIn2 :
            print("capIn2 : %d"%retIn2)
            capIn2 = cv2.VideoCapture(rtsp3) 
            #capIn2.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter.fourcc('Y','1','6',' '))
            #capIn2.set(cv2.CAP_PROP_CONVERT_RGB, False)
            continue
        ...
        Total_facedetect_tick = time.time()
        ...
        Total_facedetect_tock = time.time()
        # check detection time
        print("cam1 detection time: {}s".format(facedetect_tock1-facedetect_tick1))
        print("cam2 detection time: {}s".format(facedetect_tock2-facedetect_tick2))
        print("cam3 detection time: {}s".format(facedetect_tock3-facedetect_tick3))
        print("Total detection time: {}s".format(Total_facedetect_tock-Total_facedetect_tick))
        ...
        if len(bboxes_Out) != 0:
            ...
            if max_area >= 8000:
                ...
                if name is not None:
                    opencv_tick = time.time()
                    draw_points, points, hand_box = hand_detector(rgb)
                    opencv_tock = time.time()
                    #print("hand_runtime = {}s".format(opencv_tock - opencv_tick))
                    #hand score
                    ...
                    if handScore != 0:
                        ...
                        print("hand_score : {}".format(strScore))
